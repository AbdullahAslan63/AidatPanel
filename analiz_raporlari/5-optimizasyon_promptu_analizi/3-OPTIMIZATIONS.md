...
## AIDATPANEL v0.0.8 - OPTIMIZASYON RAPORU

**Versiyon:** 1.0  
**Tarih:** 2026-05-04  
**Kod Versiyonu:** v0.0.8  
**Hedef:** YOL_HARITASI.md v2.0 → Aşama 2 (Optimizasyon)  
...
### Genel Optimizasyon Sağlığı: 🟡 **ORTA-RİSK**

AidatPanel v0.0.8 kodu, **Faz 1 (temel yapı)** aşamasında ve temel Clean Architecture yapısı kurulmuş durumda. Ancak **15 kritik optimizasyon eksikliği** mevcut — bunların çoğu `YOL_HARITASI.md` Aşama 0 (Acil Güvenlik) ve Aşama 2 (Optimizasyon)'da ele alınacak.

### Top 3 En Yüksek Etkili İyileştirme

| Sıra | İyileştirme | Etki | Aşama |
|------|-------------|------|-------|
| 1 | **ListView.builder kullanımı** (4 konum) | 60 FPS stabilite, memory düşüşü | Aşama 0.4 |
| 2 | **Prisma N+1 query optimizasyonu** | API latency <200ms | Aşama 2.3 |
| 3 | **Image caching stratejisi** | Offline-first deneyim, hızlı yükleme | Aşama 2.1 |
...
#### F-04: DioClient Refresh Token Sonsuz Döngü Riski
- **Category:** Concurrency / Async / Reliability  
- **Severity:** High  
- **Impact:** Infinite retry, battery drain, backend DDoS riski  
- **Evidence:** `dio_client.dart:74-76` — refresh request kendi interceptor'ını çağırıyor  
- **Why inefficient:** Refresh başarısız olursa tekrar refresh denemesi (kendi kendini tetikler)  
- **Recommended fix:** Ayrı `Dio` instance (interceptor'sız) için refresh: `final _refreshDio = Dio(BaseOptions(...))`  
- **Tradeoffs/Risks:** Kod karmaşıklığı artışı (minimal)  
- **Expected impact:** Sonsuz döngü riski elimine  
- **Removal Safety:** Safe  
- **Reuse Scope:** file-local  

#### F-05: Token Expiry Süresi Yanlış (30g vs 15dk)
- **Category:** Reliability / Security  
- **Severity:** High  
- **Impact:** Auth bypass riski, sessiz 401'lar, kullanıcı deneyimi bozulması  
- **Evidence:** `auth_repository_impl.dart:47-48, 79-80, 111-112` — `Duration(days: 30)` yerine `Duration(minutes: 15)` olmalı  
- **Why inefficient:** Backend access token 15dk, client 30g sanıyor — mismatch  
- **Recommended fix:** 3 konumda `Duration(minutes: 15)`, refresh için ayrı expiry alanı  
- **Tradeoffs/Risks:** Daha sık refresh (kabul edilebilir)  
- **Expected impact:** Auth doğruluğu garanti  
- **Removal Safety:** Safe  
- **Reuse Scope:** file-local  
...
| `planning/OPTIMIZASYON_PROMPTU.md` | Bu raporun üretim prompt'u |
| `planning/AIDATPANEL.md` | Mimari referans |
