...
Bina yönetimi endpoint'leri. Tüm endpoint'ler JWT auth gerektirir.

### Endpoint'ler

| Method | Endpoint | Açıklama |
|--------|----------|----------|
| POST | `/api/v1/buildings` | Yeni bina oluştur |
| GET | `/api/v1/buildings` | Tüm binaları listele |
| GET | `/api/v1/buildings/:id` | Bina detayı getir |
...
**Bina Oluştur:**
```bash
curl -X POST http://localhost:4200/api/v1/buildings \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <access_token>" \
  -d '{"name":"Yıldız Apartmanı","address":"Atatürk Cad. No:15","city":"İstanbul"}'
```

**Bina Listele:**
```bash
curl -X GET http://localhost:4200/api/v1/buildings \
  -H "Authorization: Bearer <access_token>"
```

**Bina Güncelle:**
```bash
curl -X PUT http://localhost:4200/api/v1/buildings/<building_id> \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <access_token>" \
  -d '{"name":"Yeni Apartman Adı","address":"Yeni Adres"}'
```
...
Daire yönetimi endpoint'leri. Tüm endpoint'ler JWT auth gerektirir.

### Endpoint'ler

| Method | Endpoint | Açıklama |
|--------|----------|----------|
| GET | `/api/v1/buildings/:buildingId/apartments` | Binadaki daireleri listele |
...
**Daire Ekle:**
```bash
curl -X POST http://localhost:4200/api/v1/buildings/<building_id>/apartments \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <access_token>" \
  -d '{"number":"A-101","floor":1}'
```

**Daireleri Listele:**
```bash
curl -X GET http://localhost:4200/api/v1/buildings/<building_id>/apartments \
  -H "Authorization: Bearer <access_token>"
```
